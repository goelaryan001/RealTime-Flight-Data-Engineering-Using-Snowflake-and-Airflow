...
**Task graph:** `cleanup_stale_bronze >> bronze_ingest >> silver_transform >> data_quality_check >> gold_layer >> snowflake_load >> generate_dashboard`, defined in `dags/flight_pipeline.py`. Each stage is a `PythonOperator` calling into `scripts/<stage>.py`; file paths are passed between tasks via XCom, not the data itself.
...
- `cleanup.py` → `find_stale_files(directory, retention_days, now) -> list[Path]`: deletes `data/bronze/flights_*.json` files older than `RETENTION_DAYS` (2) before each run's bronze file is written. Only bronze needs this — silver/gold filenames are scoped to `ds_nodash` and get overwritten within the same day, so they don't accumulate.
...
**Dashboard** (`scripts/dashboard.py`): the last DAG task, running only after `snowflake_load` succeeds — queries both `ANALYTICS` views (unlimited rows, not just top N) and renders a self-contained HTML report (`render_dashboard_html`, pure function, no charting library — inline SVG bar charts built from plain geometry) to `data/dashboard/latest.html`. Regenerated every run, so it always reflects what's actually in Snowflake, not a stale snapshot. Includes a client-side country search box — `build_country_index()` merges both views into one dict, embedded as JSON in the page and filtered with plain JS, so every country is searchable even though the charts only display the top `TOP_N`. No server involved; it's all in the static file.
